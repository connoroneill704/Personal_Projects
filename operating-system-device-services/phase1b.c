...
* File: phase1b.c
* The point of this is to simulate a kernel and control processes for the 
* operating system.
*/
...
typedef struct proc{
    //represents a process on the os
    struct proc *older;
    struct proc *younger; 
    struct proc *child;   
    struct proc *parent;
...
void trampoline(){
    //trampoline function that initializes process and quits it when finished
    int psr = USLOSS_PsrGet();
    if ((psr & USLOSS_PSR_CURRENT_INT) == 0) {
        USLOSS_PsrSet(psr | USLOSS_PSR_CURRENT_INT);
    }
    int ret=table[curr_pid-1].startFunc(table[curr_pid-1].arg);
    if (strcmp(table[curr_pid-1].name, "testcase_main") == 0) {
        if (ret!=0) {
            USLOSS_Console("ERROR: testcase_main() returned non-zero value %d\n", ret);
        }
        USLOSS_Halt(ret);
    } else {
        quit(ret);
...
int join(int *status){
    //Blocks process until a child has terminated then returns status of the child
    int psr=USLOSS_PsrGet();
    if ((psr & USLOSS_PSR_CURRENT_INT)!=0) {
        USLOSS_PsrSet(psr & ~USLOSS_PSR_CURRENT_INT);
    }
    ...
    if(dead!=NULL) {
        *status=dead->exit_status;
        if (dead->younger!=NULL) {
            dead->younger->older=child->older;
        }
        if (dead->older!=NULL) {
...
__attribute__((noreturn)) void quit(int status) {
    //Function never returns, simply ends the current process and returns status
    int psr=USLOSS_PsrGet();
    proc *curr=&table[curr_pid-1];
    if (!(psr & USLOSS_PSR_CURRENT_MODE)) {
    ...
    curr->state=-2;
    int p=curr->prio-1;
    //dequeues process that has terminated
    ...
    for (int i=0;i<curr->zap_count;i++) {
        proc *zapping_proc=curr->zapped[i];
        zapping_proc->state=READY;
        int prio=zapping_proc->prio-1;
        queue[prio][queue_len[prio]++]=zapping_proc;
    }
    USLOSS_PsrSet(psr);
    dispatcher();
}
...
void zap(int pid) {
    //zaps a process at waits for it to die
    ...
    table[curr_pid-1].state=-1;
    curr_pcb->state=-1;
    int prio = curr_pcb->prio - 1;
    for (int i = 0; i < queue_len[prio]; i++) {
        if (queue[prio][i] == curr_pcb) {
            for (int j = i; j < queue_len[prio] - 1; j++) {
                queue[prio][j] = queue[prio][j + 1];
            }
            queue_len[prio]--;
            break;
        }
...
void blockMe(void) {
    //changes its own state to blocked
    int psr=USLOSS_PsrGet();
    if ((psr & USLOSS_PSR_CURRENT_INT)!=0) {
        USLOSS_PsrSet(psr & ~USLOSS_PSR_CURRENT_INT);
    }
...
void dumpProcesses() {
    //Prints debug data about process table
...
void swapTo(int pid){
    //Helper function used to switch between processes
...
void dispatcher(void) {
    //decides who gets to run and for how long based on queues
    int end=0;
    for (int j=0;j<5;j++) {
        if (queue_len[j]>0) {
...
void phase1_init(void) {
    //simulation start up function. Initialized data structures and process table
    ...
    curr_pid=1;
    //initializes init process that starts everything else
